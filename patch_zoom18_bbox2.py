...
new = '''                                val tilesDir = java.io.File(context.filesDir, "tiles/SAT/18")
                                val bounds = mutableListOf<String>()
                                if (tilesDir.exists()) {
                                    val z = 18
...
if old in kt:
    ...
    # Debug — show what we find around the tile scan
    idx = kt.find('tiles/SAT/10')
    if idx >= 0:
        print('Found SAT/10 at char', idx)
        print(repr(kt[idx-200:idx+200]))
    else:
        print('SAT/10 not found at all')
