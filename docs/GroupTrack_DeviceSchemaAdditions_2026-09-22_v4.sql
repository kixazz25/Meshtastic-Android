-- =====================================================================
-- GroupTrack — DEVICE schema additions for grouptrack_data.db   2026-09-22 v4
-- v4: DISTRIBUTED MODEL (Fred 09-22) — on a rider's tablet, other people exist only as data
--     inside the ride: rides carry organizer_name / organizer_callsign / org_name, organizer_id
--     is set only when you are the organizer; enrollments carry callsign / display_name,
--     user_id only on your own. Plus inherit_org (leader chooses at ride creation).
-- v3: the email is REQUIRED on the device's own profile (is_self) — Fred 09-22.
-- v2: users becomes the rider PROFILE (defaults that seed each ride's enrollment).
-- MARKER: DEVSCHEMA-2026-09-22-v4
-- =====================================================================
...
-- Riders = the PROFILE: this device's own rider + every rider it meets.
--   Created the first time it is needed (before creating a ride or joining a
--   ride network), seeded from ConvoyUserStore; edited after, never re-asked.
--   The profile holds DEFAULTS; each ride's enrollment starts from them and can
--   override (role, vehicle, team). callsign lives HERE, the same everywhere.
--   is_organizer = CAPABILITY (can lead). is_self = device-only.
--   EMAIL IS REQUIRED on the own profile (is_self = 1): it is the rider's identity
--   when they share data, and the address used for ride sharing. Riders met on a
--   ride have no known email, so the rule applies to is_self only.
--   Server-only V3 fields (google_id, fcm_token, terms, expires_at) are NOT here:
--   they arrive with 3.0 sign-in.
CREATE TABLE IF NOT EXISTS users (
    ...
    CHECK (is_self = 0 OR email IS NOT NULL)            -- email required on this device's own profile
);
...
CREATE TABLE IF NOT EXISTS rides (
    ...
    organizer_id            TEXT,                           -- set only when YOU are the organizer
    organizer_name          TEXT NOT NULL,                  -- every ride has a named leader (from the ride file)
    organizer_callsign      TEXT NOT NULL,
    org_id                  TEXT,                           -- your own org only; optional by design
    org_name                TEXT,                           -- from the ride file; none for independent organizers
...
--   seeded from the profile's defaults (role, vehicle, team), then overridable.
...
CREATE TABLE IF NOT EXISTS enrollments (
    ...
    user_id         TEXT,                                   -- set only on YOUR own enrollment
    callsign        TEXT NOT NULL,                          -- radio-only rider: the radio's name
    display_name    TEXT,
