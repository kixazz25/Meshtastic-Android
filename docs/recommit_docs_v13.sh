#!/bin/bash
# recommit_docs_v13.sh
# GroupTrack document management + cross-reference generation
# V13: Step 4 now folds HTML docs into GroupTrack_AllDocs.txt (v12 copied .html
...
# Usage:    bash docs/recommit_docs_v13.sh
...
echo "=== GROUPTRACK DOCS RECOMMIT v13 ==="
...
# Copy this script
cp "$DOWNLOADS/recommit_docs_v13.sh" "$DOCS/recommit_docs_v13.sh" 2>/dev/null
...
# manual) so AllDocs is complete. v12 copied .html into docs/ but never folded its
# TEXT into AllDocs. Prefer pandoc (already used for .docx above); fall back to a
# crude tag-strip via sed if pandoc is absent. Mirrors the .md loop.
for f in "$DOCS"/*.html; do
  ...
  if command -v pandoc &>/dev/null; then
    ...
    # Fallback: strip tags, drop <script>/<style> blocks, collapse blank runs
...
echo "================================================================================"
echo "Auto-generated by recommit_docs_v13.sh"
