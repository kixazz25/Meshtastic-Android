#!/usr/bin/env python3
"""
patch_area_B3_multistate_2026-08-21.py
...
def main():
    ...
    # B7: targeted -- NOT a blanket replace. The execute() wrapper this patch
    # adds legitimately contains `areaLabel = state.name`; a global substitution
    # would rewrite it to `areaLabel = areaLabel` (self-referencing parameter).
    LINE_EDITS = [
        ('            publishProgress(state.name, totalSources, sources, "running", startMs)',
         '            publishProgress(areaLabel, totalSources, sources, "running", startMs)', 3),
        ('            publishProgress(state.name, totalSources, sources,',
         '            publishProgress(areaLabel, totalSources, sources,', 1),
        ('                "for ${state.name} in ${(System.currentTimeMillis() - startMs) / 1000}s")',
         '                "for ${areaLabel} in ${(System.currentTimeMillis() - startMs) / 1000}s")', 1),
    ]
    for old, new, expect in LINE_EDITS:
        n = out.count(old)
        if n != expect:
            print("\nABORT -- line edit expected %d, found %d:\n  %s" % (expect, n, old.strip()))
            return 6
        out = out.replace(old, new)
        print("  OK  %-32s x%d -> areaLabel" % (old.strip()[:32], expect))

    # stale-reference check, now aware of what THIS patch legitimately adds:
    # the execute() wrapper reads state.bbox* (x4) and state.name (x1).
    expected = {"state.bbox": 4, "state.name": 1,
                "state.slug": 0, "state.parentState": 0, "state.gpkgUrl": 0}
    bad = []
    for tok, exp in expected.items():
        got = out.count(tok)
        if got != exp:
            bad.append("    %-20s found %d, expected %d" % (tok, got, exp))
    if bad:
        print("\nABORT -- stale `state.` references:")
        print("\n".join(bad))
        print("  (the execute() wrapper legitimately keeps state.bbox x4 + state.name x1)")
        return 5
    print("  OK  state.* references are exactly the wrapper's own")
