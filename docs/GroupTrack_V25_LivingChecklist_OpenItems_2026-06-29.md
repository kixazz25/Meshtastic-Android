...
**⛔⛔ 06-30 FIRST TASK IS DECIDED: WIRE TRACK ALIASES. Everything else waits behind it + the dupe cleanup. ⛔⛔**
...
### 📋 06-30 PLAN (revised — alias wiring leads)
1. **DUPE CLEANUP — ✅ done on Droid 1** (55 deleted, 73 hash files remain). Droid 2 keeps its own set. (Optional: resolve the 73-vs-74 row discrepancy.)
2. **WIRE TRACK ALIASES** — build the one shared dupe resolver (INSERT / DROP+delete / ALIAS+delete) and route sync, import, AND create through it. Mirror the trail ALIAS path; reuse the existing alias writer. This is the 06-30 first build.
3. **FIELD-VERIFY THE METRIC FEED** (TrackPropertiesUpdater) via the cold-launch recipe — the 06-29 test was inconclusive (track_properties still rows:0; app likely resumed old code from recents). Confirm `=== properties: N tracks written ===` and real distance/duration/speed in track_properties.
4. **FIELD-CERT CREATE** on Droid 2 (record→save→displays→`<hash>.gpx`→DB row + properties + alias if name-dupe).
5. **FIELD-CERT IMPORT** (foreign GPX → `<hash>.gpx`, DB rows, properties, aliases on dupes).
6. **THEN route-planning popup BUG A.** Gate the next AAB on all the above + popup.
