#!/bin/bash
# recommit_docs_v17.sh
# GroupTrack document management + cross-reference generation
# V17: WHERE-USED IS NOW DERIVED FROM THE FUNCTION UNIVERSE, not a hardcoded list.
#      v14/v15/v16 grepped a FROZEN ~60-symbol list from the radio/ride era
#      (startRecording, installProfileToRadio, recalcLead, createRide, ...) with
#      ZERO map/route/tile symbols -- so where_used_raw.txt reported ~254 sites and
#      was BLIND to fit, insertRoute, shouldInterceptRequest, segmentCells,
#      RouteDraftStore, and every current-work symbol. It reported success while
#      being wrong. V17 extracts identifiers from function_universe_raw.txt (just
#      generated) and searches the mesh source for those -- so the index always
#      covers current code with no hand-maintained list. Also: fixed the cosmetic
#      banner (said v14). The V16 pathspec-scoped commit is UNCHANGED.
...
# Usage:    bash docs/recommit_docs_v17.sh
...
echo "=== GROUPTRACK DOCS RECOMMIT v17 ==="
...
echo "--- Step 1: Generating where-used (fast, V17: symbols derived from function universe) ---"

# V17: build the search symbol set FROM function_universe_raw.txt instead of a
# hardcoded list. Extract declared identifier names (fun/val/var/object/class),
# keep distinctive ones (len>=4, drop common noise), grep the mesh source for them.
SYMS=$(sed -E 's/.*(fun|val|var|object|class)[[:space:]]+([A-Za-z_][A-Za-z0-9_]*).*/\2/' \
        "$DOCS/function_universe_raw.txt" \
      | grep -E '^[A-Za-z_][A-Za-z0-9_]{3,}$' \
      | grep -vE '^(the|and|for|this|null|true|false|return|value|index|count|item|list|name|type|data|text|size|width|color|state|Companion|TAG|context|scope|result|error|label|title|content|entry|field|param|args|flag)$' \
      | sort -u)
SYM_COUNT=$(printf '%s\n' "$SYMS" | grep -c . )
# Chunk into grep -f pattern file (an alternation would blow the arg limit at thousands of symbols)
SYM_PATTERN_FILE="$DOCS/.where_used_symbols.tmp"
printf '%s\n' "$SYMS" > "$SYM_PATTERN_FILE"
echo "  derived $SYM_COUNT search symbols from function universe"

grep -rnf "$SYM_PATTERN_FILE" \
  "$MESH_SRC/" \
  --include="*.kt" \
  | grep -v "^.*: *\/\/" \
...
cp "$DOWNLOADS/recommit_docs_v17.sh" "$DOCS/recommit_docs_v17.sh" 2>/dev/null
...
echo "================================================================="
echo "Auto-generated by recommit_docs_v17.sh"
...
echo ""
echo "================================================================="
echo "RECOMMIT V17 COMPLETE -- $(date '+%Y-%m-%d %H:%M')"
echo "================================================================="
