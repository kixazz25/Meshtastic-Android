#!/usr/bin/env python3
"""
nav_pass4_ranked_2026-08-22.py
...
EXPLORE_MAX = 200.0            # explore this far, then rank
N_ROUTES    = 4
TRIES       = 2500
PENALTY     = 2.2              # multiplier on edges already used by a chosen route
REPORT_CUTS = [60, 80, 100, 140, 200]
...
def main():
        ...
        for _ in range(TRIES):
            ...
            while True:
                ...
                for k in left:
                    ...
                    if mi + s1 + back > EXPLORE_MAX * (0.55 if phase == 0 else 1.0):
                        continue
    ...
    for i in range(N_ROUTES):
        ...
        cand = [r for r in pool
                if all(len(r["eset"] & q["eset"])/max(len(r["eset"] | q["eset"]),1) < 0.75
                       for q in picks)]
        if not cand:
            print("  (no sufficiently different alternative)"); break
