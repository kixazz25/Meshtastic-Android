# GroupTrack V2.5 — Living Checklist (CONSOLIDATED, full problem definitions) — 2026-06-10
...
### 1.7 — Route-builder cosmetic fixes — OPEN
(a) Empty In-Progress picker shows only "Cancel" — add "No in-progress routes yet". (b) The "Name required" red hint reuses the routeNameTaken flag, so it wrongly reads "name is taken" — add a distinct message for blank-name vs taken-name. (Source: 06-06 v3 §F.)
...
### 3.4 — Convoy waypoint-drop pipeline present but doesn't fire — OPEN
...
### 3.5 — Convoy `?` help button misplaced — OPEN
The convoy `?` help button shipped INSIDE `if (convoyState.hasLost ...)`, so it only appears when a contact is lost. Move it beside the QUEUES button (ConvoyScreen.kt:1359 locked top-right Surface, Text("QUEUES")@1368), out of the hasLost conditional. Read sed -n '1357,1373p' to match the QUEUES Surface style. Planning `?` is correct/always-visible. (Source: 06-07 session/memory.)
...
### 3.8 — Waypoint marker shape DECISION — OPEN
Triangle (original) vs round pin (shipped). Decide and make consistent. (Source: master §F.)

### 3.9 — Direction arrows on track/trail lines — OPEN
Add direction-of-travel arrows along track/trail polylines. (Source: master §F.)
...
### 4.6 — Streaming-parse geometry over-capture VERIFY (pre real-device ship) — VERIFY
...
### 6.6 — Duplicate AlertDialog import (ConvoyScreen.kt 34 & 85) — OPEN (tidy)
...
### 8.1 — ANR #2 osmdroid tile-cache scan (MUST resolve before launch) — OPEN
~60GB tile-cache scan on the main thread at onCreate causes a 20–43s freeze (P10_T). Disable osmdroid cache trimming. Launch-blocking. (Source: master §G.)

### 8.2 — ANR #1 MANAGE_EXTERNAL_STORAGE startup blocks main thread — OPEN
Startup blocks the main thread on the storage permission. Defer file-dependent init until the permission is confirmed. (Source: master §G.)

### 8.3 — ANR Type 2 Input Dispatching Timeout — VERIFY
May share the osmdroid root cause; verify after 8.1. (Source: master §G.)

### 8.4 — Strip diagnostic logs before AAB — OPEN (AAB gate)
Remove debug Log.d before the release bundle: ConvoyScreen tracedLen (~555) + S2 tracedLen (~716) + the planning "S2P tracedLen" log on ConvoyMapViewerScreen onMapTap (~490). (Source: 06-06 v3 §E; 06-10.)

### 8.5 — SpecifyForegroundServiceType lint — OPEN (AAB gate)
androidx.work SystemForegroundService needs android:foregroundServiceType in AndroidManifest.xml. Pre-existing, lint-only, build succeeds — fix before Play submission. (Source: RouteToolbar_CleanupItems_v3; master §G.)

### 8.6 — versionCode bump + signing confirm — OPEN (AAB gate)
Bump versionCode (Droid builds seen to 29320598; fallback AAB was 29320600 on Play Internal; must monotonically increase; versionName cosmetic). bundleGoogleRelease honors -Pandroid.injected.version.code=NNNNN first. Confirm signing (Play vs local APK use DIFFERENT keys). (Source: master §G; 06-06 v3 §E; memory.)

### 8.7 — About / Attribution screen — OPEN
GPL / Leaflet / Esri attribution screen for Play compliance. (Source: master §G.)

### 8.8 — lintVital ServiceKeepAlive tidy — OPEN
Clean up the lintVital ServiceKeepAlive warning. (Source: master §G.)
