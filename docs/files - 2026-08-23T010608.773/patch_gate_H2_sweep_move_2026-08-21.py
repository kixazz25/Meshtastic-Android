#!/usr/bin/env python3
"""
patch_gate_H2_sweep_move_2026-08-21.py
...
def main():
    ...
    # ⚠ SCOPED CHECK. There is a LEGITIMATE renameTo elsewhere in this file
    # (~:510, the atomic tmp->dest rename on the OSM download path) which works
    # because it stays within one directory. A file-wide check flags that, and
    # flags this script's own explanatory comment. Guard the EXPRESSION, not the
    # token: the sweep's specific call must be gone.
    if "if (f.renameTo(target))" in out:
        print("\nABORT -- the sweep still calls renameTo. No write."); return 5
    print("  OK  sweep no longer calls renameTo (the ~:510 rename is untouched)")
