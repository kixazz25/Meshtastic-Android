#!/usr/bin/env python3
"""
nav_pass7_broken_ridge_2026-08-22.py
...
OUT = r"D:\nav_test\out"
SET_NAME = "Broken Ridge"      # Fisher Wash Rd x railroad, Modena UT

# Broken Ridge -- the intersection of Fisher Wash Road and the railroad at
# Modena, far western Utah, close to the Nevada line. Coordinate supplied by
# Fred rather than guessed: a start point a mile out would anchor the whole
# exploration in the wrong place.
#
# THIRD corridor. The Strip was BLM and 77% unnamed; Panguitch was Dixie NF and
# richly named (Trail Creek, Upper Bear Valley Road, Showalter Mountain) and
# produced the best routes so far -- 13 features at 25% retrace. Modena is
# different again: high desert, rail corridor, and far from a forest. Whether
# the model holds a third time is the point of running it.
BAR10 = (38.1309, -113.44312)
...
SPEED_MPH = (12.0, 18.0)
LAT_S, LAT_N, LON_W, LON_E = 37.85, 38.42, -113.85, -113.05
...
def main():
    ...
    cur = con.cursor()
    print("="*78)
    print("PASS 7 -- FREE LOOP FROM BROKEN RIDGE   %s   explore to %.0f mi"
    ...
    terms = [n_bar] + [k for k in poi_at if k != n_bar]
        ...
        for _ in range(TRIES):
            ...
            while True:
                ...
                for k in left:
                    ...
                    # the only constraint is getting home inside the target
                    if mi + s1 + back > target: continue
