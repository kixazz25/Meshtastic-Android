#!/usr/bin/env python3
"""
nav_sidecar_narrative_2026-08-22.py
...
def narrate(d):
    ...
    for f in seq:
        ...
        stops.append("mile %.0f  %s -- %s%s"
                     % (m if m else 0, f["name"], PLAIN.get(f["fclass"], f["fclass"]), near))
    ...
    warn = []
    if s.get("retrace_pct", 0) > 25:
        warn.append("About %.0f%% of the ride retraces ground you have already "
                    "covered -- the trail network out to Toroweap is a spur, not "
                    "a loop, so the way back shares much of the way out."
                    % s["retrace_pct"])
    if cauts:
