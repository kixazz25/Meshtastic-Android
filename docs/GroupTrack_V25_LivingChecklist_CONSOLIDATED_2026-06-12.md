...
### 11.2 — Trail / Route DETAIL via SELECT/Edit list (not map-tap) — IN FLIGHT (closes when [2h] card builds)
Provide trail/route detail from the SELECT/Edit list, not only via map-tap. (Source: master §D.) **2026-06-12: being delivered by the [2h] detail card.** The live SEL/Edit detail surface is `ArtifactListPanel.kt` (hosted ConvoyScreen:1651 + ConvoyMapViewerScreen:1221); its detail dialog has been rebuilt (cram2, applied-unbuilt) as the two-column card reading the spatial row (full-data) + data-DB aliases. Closes when pieces 3+4 build and device-test. Convoy = read-only; planning = editable (preserve asymmetry).
