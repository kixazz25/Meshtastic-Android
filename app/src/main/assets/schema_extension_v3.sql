-- GroupTrack Extension Schema v3
-- File: grouptrack_data.db
-- v3 = v1 extension schema (all property/queue/log/config tables + views) UNCHANGED,
--      EXCEPT artifact_aliases gains alias_date (UTC day) + geom_hash + UNIQUE(artifact_type, geom_hash, alias_date).
-- Built fresh on install after one-time delete. proximity_config re-seeds here.
...
-- 2.6 ARTIFACT ALIASES  (v3: + alias_date, + geom_hash, + UNIQUE(artifact_type, geom_hash, alias_date))
CREATE TABLE IF NOT EXISTS artifact_aliases (
    ...
    alias_date          TEXT,
    geom_hash           TEXT,
    UNIQUE(artifact_type, geom_hash, alias_date)
);
...
SELECT t.trail_id,
       ...
       tp.owner_steward, tp.status, tp.shared
FROM trails t
