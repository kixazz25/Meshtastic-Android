...
class ConvoyEngine {package com.geeksville.mesh.convoy
    ...
    object ConvoyEngine {
        ...
        fun computeSpan(lead: ConvoyNode?, tail: ConvoyNode?): Float {
            ...
            return haversineMiles(lead.latitude, lead.longitude,
            ...
            return nodes.map { node ->
                ...
                node.copy(
                    ...
                    feetToNodeAhead = if (ahead != null)
                        haversineMiles(node.latitude, node.longitude,
                            ahead.latitude, ahead.longitude) * 5280f else 0f,
                    feetToNodeBehind = if (behind != null)
                        haversineMiles(node.latitude, node.longitude,
                            behind.latitude, behind.longitude) * 5280f else 0f,
                    milesToLead = if (lead != null)
                        haversinemiles(node.latitude, node.longitude,
                            lead.latitude, lead.longitude) else 0f,
                    milesToTail = if (tail != null)
                        haversineMiles(node.latitude, node.longitude,
                            tail.latitude, tail.longitude) else 0f
                )
            }
        }
        ...
        fun computeLeadTrackColors(
            ...
            return segments.map { seg ->
                ...
                if (proj in minProj..maxProj) {
                    val closest = nodes.minByOrNull {
                        abs((it.latitude * dy + it.longitude * dx) - proj)
                    }
        ...
        private fun haversineMiles(lat1: Double, lon1: Double,
