...
# Fix HudCard padding - remove start=16.dp gap
for i, l in enumerate(lines):
    if '.padding(start = 16.dp, bottom = 12.dp)' in l:
        print(f'HudCard padding at line {i+1}: {l.rstrip()}')
        lines[i] = l.replace('.padding(start = 16.dp, bottom = 12.dp)', '.padding(start = 0.dp, bottom = 12.dp)')
        print(f'Fixed: {lines[i].rstrip()}')
        break

# Find GroupHud boundaries
grouphud_start = None
grouphud_end = None
for i, l in enumerate(lines):
    if 'fun GroupHud(' in l and grouphud_start is None:
        grouphud_start = i - 1  # include @Composable line
